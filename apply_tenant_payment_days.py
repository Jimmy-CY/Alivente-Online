...
@media (max-width: 768px) {
    ...
    /* Must out-specify `.pd-table tbody td::before`, which otherwise fills this
       cell with an empty data-label and leaves a bare chevron with no caption. */
    .pd-table tbody td.pd-col-toggle::before { content: "Every payment"; }

    /* Undo the card treatment for the nested detail table. Everything above
       is written as `.pd-table tbody td ...`, which reaches these cells too -
       so without this the four columns stack into four right-aligned lines
       with the headers orphaned above them. It stays a real table: four short
       columns fit a phone, and one row per payment is the point of it. */
    .pd-detail td { display: block; padding: 0 !important; }
    ...
    .pd-detail .pd-detail-table td.pd-num { text-align: right; }
    /* The card layout injects a label before every cell; not here. */
    .pd-detail .pd-detail-table td::before,
    .pd-detail .pd-detail-table th::before { content: none; }
    /* The card layout right-aligns cell contents; the detail block is prose
       plus a table and reads left, like everywhere else. */
