...
Contains 26 functions in three loose groups:
...
def fsr_notification(request):
    # Kept inline to avoid a circular import with pages.email_utils.
...
def comments_report(request):
    ...
    for comment in comments:
        ...
        report_data.append({
            'comment_id': comment.issues_details_id,        # needed for delete
...
def delete_comment(request, comment_id):
    ...
    # Properly redirect back with period parameter
...
def lease_agreements(request):
    # Local import: 'print_lease' is a root-level report module.
    import print_lease
...
def title_deeds(request):
    # Local import: 'print_title' is a root-level report module.
    import print_title
    prop = request.POST.get('propname')
    rep_output = request.POST.get('d_e')
    if request.user.is_authenticated:
...
def prop_rep(request):
    # Local import: 'print_prop' is a root-level report module.
    import print_prop
    prop = request.POST.get('propname')
    rep_output = request.POST.get('d_e')
    if request.user.is_authenticated:
...
def tenant_rep(request):
    # Local import: 'print_tenant' is a root-level report module.
    import print_tenant
    prop = request.POST.get('propname')
    rep_output = request.POST.get('d_e')
    if request.user.is_authenticated:
...
@permission_required('auth.can_access_issues', raise_exception=True)
def fsr_rep(request):
    # Local import: 'fsr' is a root-level report module (distinct from the fsr view).
    import fsr
...
def issues_rep(request):
    # Local import: 'issues' here is a root-level report module, NOT the
    # issues model imported at module top - hence the deliberate local scope.
    import issues
...
@login_required
@permission_required('auth.can_access_issues', raise_exception=True)
def issues_analysis_data(request):
    """JSON feed for the Issues Analysis modal. Ships every issue (property and
    year filtering plus all aggregation happen client-side, so the charts and
    their drill-downs always reconcile) with the property list. Resolved = status
    'Resolved'; everything else counts as open. The 1900-01-01 sentinel
    resolution date is treated as 'no date'. Days-to-resolve and open age are in
    calendar days."""
    _sentinel = date(1900, 1, 1)
    _today = date.today()
    props_qs = props.objects.all().order_by('prop_country', 'prop_name')
    prop_names = {p.prop_id: p.prop_name for p in props_qs}
    rows = []
    years = set()
    for i in issues.objects.all():
        logged = i.issues_date_logged
        res = i.issues_resolution_date
        if res == _sentinel:
            res = None
        status = (i.issues_status or '').strip() or 'Issue'
        is_res = (status == 'Resolved')
        dtr = None
        if is_res and res and logged:
            dtr = (res - logged).days
            if dtr < 0:
                dtr = 0
        age = None
        if not is_res and logged:
            age = (_today - logged).days
            if age < 0:
                age = 0
        if logged:
            years.add(logged.year)
        rows.append({
            'id': i.issues_id,
            'prop': prop_names.get(i.prop_id, '\u2014'),
            'prop_id': i.prop_id,
            'head': i.issues_heading or '(no heading)',
            'desc': i.issues_description or '',
            'logged': logged.isoformat() if logged else None,
            'status': status,
            'is_resolved': is_res,
            'resolution': res.isoformat() if res else None,
            'days_to_resolve': dtr,
            'age_days': age,
            'url': reverse('fsr_details', args=[i.issues_id]),
        })
    return JsonResponse({
        'properties': [{'id': p.prop_id, 'name': p.prop_name} for p in props_qs],
        'years': sorted(years),
