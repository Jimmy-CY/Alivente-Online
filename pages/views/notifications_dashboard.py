...
NOT to be confused with pages/views/notifications.py, which contains
notification *settings* views (admin/personal notification preferences).
This module is the *dashboard data* layer.

Contains 7 functions:
  notifications_dashboard(request)  - Django view (AJAX-aware) for the
                                       notifications dashboard page.
                                       Returns JSON on XHR, template
                                       on regular page load.
  get_notification_data()           - Main aggregator returning all
                                       notification counts and detail
                                       rows. Used by this view AND by
                                       pages/views/home.py for the
                                       "Today" panel.
  get_expenses_waiting_approval(cursor)  - SQL helper
  get_expenses_waiting_payment(cursor)   - SQL helper
  get_expiring_leases(cursor, today)     - SQL helper
  get_declined_renewals(cursor, today)   - SQL helper
  get_overdue_invoices(cursor, today)    - SQL helper

The 5 SQL helpers use raw mysql.connector queries (legacy from when these
ran inside a management command). A future cleanup pass could migrate
