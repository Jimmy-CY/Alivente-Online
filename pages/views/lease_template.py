...
split (section ### LEASE TEMPLATE GENERATOR ###).

Functions (9, all lease-related)
--------------------------------
- is_superuser                  : Tiny User predicate helper. Appears
                                  unused (no callers found in the views
                                  modules) but kept verbatim in case
                                  templates or URL patterns reference it.
- generate_lease_agreement_view : Main lease-generation form view and
                                  POST handler.
- get_ordinal_day               : "1st", "2nd", "3rd", etc.
- number_to_words_greek         : integer -> Greek words.
- format_date_greek             : date -> "1η Ιανουαρίου 2026".
- translate_to_greek            : Large English->Greek dictionary for
                                  property / furniture / keys terms.
- generate_lease_document       : DocxTemplate-based generator.
- prepare_lease_template_data   : Large field-mapping helper.
- create_basic_lease_document   : python-docx fallback when no template
                                  file is present.

Note: a 10th function (spell_check_instructions) was originally
extracted here but was misplaced (it spell-checks recipe instructions,
not lease content) and was relocated to the recipe views in a
follow-up commit.
"""
