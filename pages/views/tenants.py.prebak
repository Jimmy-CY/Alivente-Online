...
Contains 9 functions:
  tenant_page             - list view with property/tenant/status filters
  tenant_add              - add form
  tenant_edit             - edit form
  tenant_commit           - add save (uses TenantForm)
  tenant_edit_commit      - edit save (uses TenantForm)
  tenant_lease_agreement  - upload/delete lease agreement document
  lease_timeline_view     - lease timeline UI (per-property tenant timeline)
  duplicate_tenant_view   - duplicate tenant for renewal/new lease
  delete_tenant_view      - delete tenant (signals handle vacancy cleanup)

URL patterns remain registered in pages/urls.py.
Tenant deletion relies on the post_save / post_delete handlers in
manage_vacancy_periods.py for vacancy period cleanup.

Indentation note: this module preserves the mixed tab/4-space indentation
of the legacy source. tenant_add, tenant_edit, and tenant_edit_commit use
TAB indentation; the other 6 use 4 spaces. A future cleanup pass can
normalize to PEP 8 four-space.
"""
