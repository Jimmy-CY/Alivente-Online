...
class Command(BaseCommand):
    ...
    def report(self, p, years, cur):
        ...
        # ---------- 3. THE CHECK ----------
        w('')
        w('3) CHECK  (does %d in the P&L match your current live budget?)' % cur)
        live_total = 0.0
        for e in exp_rows:
        ...
        for e in exp_rows:
            resolved_cur += sum(self.resolved_year(p, H.KIND_BUDGET, e.expense_id, cur,
                                                    live_row=e, live_prefix='expense'))
        diff = live_total - resolved_cur
        w('   Live budget cells total : %s' % money(live_total))
        w('   Resolved %d total       : %s' % (cur, money(resolved_cur)))
        if abs(diff) < 0.5:
            w('   MATCH ✔  — your latest budget is fully in effect for %d.' % cur)
        else:
            w('   DIFFER by %s — part of %d still uses an earlier dated figure '
              '(correct when a change took effect mid-year).' % (money(abs(diff)), cur))
